...
def compute_rvi(mean, harvest_date):
    '''
...
def compute_rvi_stats(rvi_smooth):
    ...
    harvest_date = (pd.to_datetime(harvest_date)- rvi_handling.index[0]).days
    harvest_val = 0
...
def get_all_features(crop_yield_data):
    ...
    for ind, row in crop_yield_data.iterrows():
        if debug_stop > 2:
            break
        debug_stop+=1
        mean_row = access_sentinel_data(row["Longitude"], row["Latitude"])
        mean_row = compute_rvi(mean_row, row["Date of Harvest"])
        all_features.append(compute_rvi_stats(mean_row))
